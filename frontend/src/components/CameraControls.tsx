...
export default function CameraControls({ className = "" }: Props) {
  const { cameraStatus, isLoading, switchCameraSource, toggleStream, refreshStatus } = useCameraControl();
  ...
  const handleCameraSwitch = async (source: string) => {
    ...
    try {
      ...
      setSwitchError(`Erro ao trocar para ${source}: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
    }
  };
  ...
  const handleStreamToggle = async () => {
    ...
    try {
      ...
      setStreamError(`Erro ao ${cameraStatus?.stream_enabled ? 'parar' : 'iniciar'} stream: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
    }
  };
  ...
  const getStreamButtonIcon = () => {
    ...
    return cameraStatus?.stream_enabled ? <VideoOff className="w-4 h-4" /> : <Video className="w-4 h-4" />;
  };
  ...
  return (
    ...
    >
          ...
          <div className="flex items-center gap-2">
            <div
              className={`w-3 h-3 rounded-full ${
                cameraStatus?.stream_enabled ? "bg-green-400 animate-pulse" : "bg-red-400"
              }`}
            />
      ...
      <div className="mb-6">
        <h4 className="text-sm font-medium text-gray-300 mb-3">Trocar Câmera</h4>
      ...
      <div className="mb-4">
        <h4 className="text-sm font-medium text-gray-300 mb-3">Controle de Stream</h4>
        <button
      ...
      {cameraStatus?.current_source === "ip_camera" && (
        <div className="p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
          <div className="text-xs text-blue-300 font-medium mb-1">IP Camera URL</div>
