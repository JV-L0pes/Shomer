...
function VideoStream({ src }: Props) {
  ...
  const updateStats = useCallback(() => {
    ...
    if (now - fpsCalc.lastTime >= 1000) {
      const fps = ((frameCountRef.current - fpsCalc.lastCount) * 1000) / (now - fpsCalc.lastTime);
  ...
  useEffect(() => {
    ...
    const handleError = () => {
      ...
      console.error("Erro no stream MJPEG:", src);
    };
    ...
    >
        ...
        <div className="absolute inset-0 bg-[#020617] flex items-center justify-center z-30">
          <div className="text-center max-w-md px-6">
            ...
            <p className="text-gray-400 text-sm mb-4">
              O stream está conectado mas aguarda liberação. Use os controles de câmera para iniciar o stream.
            </p>
      ...
      {hasError && (
        <div className="absolute inset-0 bg-[#020617] flex items-center justify-center z-30">
          <div className="text-center max-w-md px-6">
            ...
            <p className="text-gray-400 text-sm mb-4">
              Não foi possível conectar ao stream MJPEG. Verifique se o
      ...
      <img
        ...
        className={`w-full block transition-all duration-300 ${
          isLoading || hasError || connectionStatus === "waiting" ? "opacity-0" : "opacity-100"
        }`}
        style={{
          maxHeight: "600px",
          objectFit: "contain",
