...
class VisualDetector:
    ...
    def __init__(self, src=0):
        print(f"🎨 [VISUAL] Inicializando detector ultra-otimizado - fonte: {src}")
        ...
        print("✅ [VISUAL] Detector ultra-otimizado iniciado")
    
    def _setup_camera(self, src):
        """Configuração ultra-otimizada da câmera."""
        print("📹 [VISUAL] Configurando câmera ultra-otimizada...")
            ...
            try:
                ...
                if cap.isOpened():
                    ...
                    if ret and frame is not None:
                        self.cap = cap
                        print(f"✅ [VISUAL] Usando: {name}")
                        break
                    cap.release()
            except Exception as e:
                print(f"⚠️  [VISUAL] {name} falhou: {e}")
        ...
        print(f"📺 [VISUAL] Resolução: {width}x{height} @ {fps:.1f}fps")
    ...
    def _setup_models(self):
        ...
        try:
            print("🤖 [VISUAL] Carregando modelos de IA ultra-otimizados...")
            ...
            try:
                ...
                self.face_analyzer.prepare(ctx_id=-1, det_size=(320, 320))  # Tamanho menor
                
                # Warmup
                _ = self.face_analyzer.get(dummy)
                
                print("✅ [VISUAL] InsightFace pronto para detecção facial")
                self.face_available = True
                
            except Exception as e:
                print(f"⚠️  [VISUAL] InsightFace indisponível: {e}")
                self.face_available = False
            
            self.models_ready = True
            print("🎨 [VISUAL] Modelos prontos - feedback visual ativo!")
            
        except Exception as e:
            print(f"❌ [VISUAL] Erro nos modelos: {e}")
            self.yolo_available = False
            self.face_available = False
    ...
    def _start_threads(self):
        ...
        print("🧵 [VISUAL] Threads ultra-otimizadas iniciadas")
    ...
    def _capture_loop(self):
        ...
        while self.running:
            try:
                ...
                print(f"❌ [VISUAL] Erro na captura: {e}")
    ...
    def _detection_loop(self):
        ...
        while self.running:
            try:
                ...
                print(f"❌ [VISUAL] Erro na detecção: {e}")
                time.sleep(0.001)  # Sleep mínimo
    ...
    def _detect_people_with_boxes(self, frame) -> Tuple[List[Tuple], int]:
        ...
        try:
            ...
            print(f"⚠️  [VISUAL] Erro detecção pessoas: {e}")
            return [], 0
    ...
    def _detect_faces_with_boxes(self, frame) -> Tuple[List[Tuple], int]:
        ...
        try:
            ...
            print(f"⚠️  [VISUAL] Erro detecção rostos: {e}")
            return [], 0
    ...
    def _generate_test_frame(self):
        ...
        cv2.rectangle(frame, (100, 85), (140, 100), COLORS['person_label_bg'], cv2.FILLED)
        cv2.putText(frame, 'P1', (105, 97), cv2.FONT_HERSHEY_SIMPLEX, 0.5, COLORS['person_text'], 1)
        
        # Rosto simulado
        cv2.rectangle(frame, (120, 120), (180, 180), COLORS['face_box'], 2)
        cv2.rectangle(frame, (120, 105), (150, 120), COLORS['face_label_bg'], cv2.FILLED)
        cv2.putText(frame, 'F1', (125, 117), cv2.FONT_HERSHEY_SIMPLEX, 0.4, COLORS['face_text'], 1)
        
        # Texto informativo
        cv2.putText(frame, "CAMERA DE TESTE - ULTRA OTIMIZADO", (180, 50),
    ...
    def detect_and_annotate(self, frame):
        ...
        try:
            ...
            for i, (x1, y1, x2, y2) in enumerate(people_boxes):
                ...
                cv2.rectangle(annotated, (label_bg_x1, label_bg_y1), (label_bg_x2, label_bg_y2), 
                             COLORS['person_label_bg'], cv2.FILLED)
                
                # Texto do label
                cv2.putText(annotated, label, (x1 + 4, y1 - 4),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, COLORS['person_text'], 2)
            ...
            for i, (x1, y1, x2, y2) in enumerate(face_boxes):
                ...
                cv2.rectangle(annotated, (label_bg_x1, label_bg_y1), (label_bg_x2, label_bg_y2), 
                             COLORS['face_label_bg'], cv2.FILLED)
                
                # Texto do label
                cv2.putText(annotated, label, (x1 + 3, y1 - 3),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, COLORS['face_text'], 1)
            ...
            print(f"❌ [VISUAL] Erro na anotação: {e}")
            return frame
    ...
    def _add_visual_info(self, frame, results):
        ...
        info_width = 300
        info_height = 140
        cv2.rectangle(frame, (10, 10), (info_width, info_height), COLORS['info_bg'], cv2.FILLED)
    ...
    def _update_detection_tracker(self, people_count, faces_count):
        ...
        if recent_detections:
            self.detection_tracker["detections_per_second"] = sum(recent_detections) / len(recent_detections)
    ...
    def get_performance_stats(self):
        ...
        return {
            ...
            "detection_rate": {
                "detections_per_second": self.detection_tracker["detections_per_second"],
            ...
            "tracking_stats": {
                ...
                "session_duration": (datetime.now() - self.person_tracking["session_start"]).total_seconds()
            }
        }
    ...
    def _calculate_detection_efficiency(self):
        ...
        if self.current_fps > 0:
            detection_efficiency = (self.detection_tracker["detections_per_second"] / self.current_fps) * 100
            return min(100, max(0, detection_efficiency))  # Limitar entre 0-100%
        return 0
    
    def stop(self):
        """Para o detector."""
        print("🛑 [VISUAL] Parando detector ultra-otimizado...")
        self.running = False
        ...
        print("✅ [VISUAL] Detector ultra-otimizado parado")
    ...
    def _track_persons(self, people_boxes):
        ...
        for i, bbox in enumerate(people_boxes):
            ...
            if person_id not in self.person_tracking["active_persons"]:
                ...
                print(f"👤 [TRACKING] Pessoa {self.person_tracking['person_counter']} entrou na cena")
            else:
                # Pessoa já conhecida, atualizar última vez vista
                self.person_tracking["active_persons"][person_id]["last_seen"] = current_time
        ...
        for person_id, person_data in self.person_tracking["active_persons"].items():
            if person_id not in current_persons:
                time_since_last_seen = (current_time - person_data["last_seen"]).total_seconds()
                
                if time_since_last_seen > 3.0:  # 3 segundos sem detectar = saiu
                    persons_to_remove.append(person_id)
                    
                    # Calcular tempo de permanência
                    duration = (person_data["last_seen"] - person_data["entry_time"]).total_seconds()
                    ...
                    print(f"👋 [TRACKING] Pessoa {person_data['person_id']} saiu da cena (permaneceu {duration:.1f}s)")
